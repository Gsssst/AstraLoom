...
### Requirement: Workspace Detail Uses Shared Page Shell

The workspace detail page SHALL use the shared page shell for object title, subtitle, actions, width, and content spacing.

#### Scenario: User opens workspace detail
- **WHEN** a workspace detail page renders
- **THEN** it uses the shared page shell with the workspace name as the page title when available
- **AND** back/module navigation commands render as page actions.
