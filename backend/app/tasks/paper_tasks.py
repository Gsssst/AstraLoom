...
@celery_app.task(bind=True, name="repair_low_quality_tables")
def repair_low_quality_tables_task(self, limit: int = 5):
    """Run high-fidelity low-quality table repair as a tracked maintenance job."""
    import asyncio

    from app.db.session import AsyncSessionLocal
    from app.services.maintenance_jobs import run_low_quality_table_repair, table_repair_progress_payload
    ...
    async def _run():
        async with AsyncSessionLocal() as db:
            try:
                ...
                return await run_low_quality_table_repair(db, limit=limit, progress=progress)
            except Exception as exc:
                await db.rollback()
                failure = table_repair_progress_payload(
                    status="failed",
                    total=0,
                    processed=0,
                    success=0,
