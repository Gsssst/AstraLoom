...
LATEX_PREVIEW_DIR = Path("/tmp/astraloom-latex-previews")
...
class LatexProcessor:
    ...
    def inspect_submission_template(self, filename: str, content: bytes) -> dict:
        ...
        document_options = self._extract_document_options(source_text)
    ...
    def _extract_document_options(self, tex_content: str) -> list[str]:
        match = re.search(r'\\documentclass(?:\[([^\]]*)\])?\{[^}]+\}', tex_content or "")
    ...
    def render_to_tex(self, project_title: str, sections: List[dict],
                      template: str = "article", render_options: dict | None = None) -> str:
        """将写作项目渲染为 .tex 文件。"""
        options = dict(render_options or {})
        layout = options.get("layout") or "single_column"
        document_class = (options.get("document_class") or template or "article").strip() or "article"
        document_options = [
            str(item).strip()
            for item in (options.get("document_options") or [])
            if str(item).strip()
        ]
        if layout == "double_column" and "twocolumn" not in document_options:
            document_options.append("twocolumn")
        if layout != "template":
            document_class = template or "article"
        document_option_text = f"[{','.join(document_options)}]" if document_options else ""
        ...
        lines = [
            r"\documentclass" + document_option_text + "{" + document_class + "}",
            r"\usepackage[utf8]{inputenc}",
            *[r"\usepackage{" + package + "}" for package in package_names],
            "",
            r"\title{" + project_title + "}",
            r"\author{Generated by AstraLoom}",
            r"\date{\today}",
            "",
            r"\begin{document}",
            r"\maketitle",
